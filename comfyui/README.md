...
doors onto it: these four nodes, and an HTTP service anything else can call. Both doors run the same
...
direction saved on this ComfyUI. Pick one and its whole text drops into the box. From there, change
a line, cut half of it, or leave it as it is.
...
got written and the report of what happened. It ships with an empty tray, so it runs on a prompt
