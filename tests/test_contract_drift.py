...
from comfyui import contract as PACK
from comfyui import tray as T
from comfyui.h3ir_client import ServiceError, fetch_contract, payload_shape

REPO = pathlib.Path(__file__).resolve().parents[1]
PACK_DIR = REPO / "comfyui"
...
def test_the_generated_copies_are_what_this_compiler_publishes():
    ...
    This is the test that survives the split, and it survives it by getting BETTER: after the split
    it runs in the pack's repository against the `open-h3-ir` the pack depends on, which is the
    released compiler rather than a sibling working tree. Until then it runs against this checkout,
...
def test_the_snapshot_the_pack_compares_against_is_the_file_it_ships():
    """`comfyui/contract.py` reads its snapshot at import from a file packaged beside it. A pack
...
def test_the_version_the_pack_tells_people_to_install_is_the_one_that_carries_the_contract():
    ...
    somebody to install something that will not fix their problem. `v0.2.0` was the last release
    without it, so the working tree's own version is what the note has to say.
    """
    version = re.search(r'^version = "([^"]+)"$',
                        (REPO / "pyproject.toml").read_text(encoding="utf-8"), re.MULTILINE)
    assert version, "pyproject no longer states a version, so this comparison is blind"
    assert PACK.FIRST_PUBLISHING_RELEASE == version.group(1), (
        f"the pack tells people to install {PACK.FIRST_PUBLISHING_RELEASE} and this tree is "
        f"{version.group(1)}. Whichever release first carries GET /v1/contract is the one the note "
        "must name.")
...
def test_the_contract_can_be_read_from_a_compiler_in_the_same_python():
    ...
    **What this proves and what it cannot.** In this repository `import h3ir` resolves to the
    sibling folder, not to an installed wheel, so this proves the wiring -- that the function finds
    a compiler, calls it, and returns something a comparison can use -- and says nothing about
    whether a real `pip install open-h3-ir` works. That is a fact only the split can establish, and
    claiming it here would be the kind of green test that proves nothing.
    """
...
def test_a_picture_that_replaces_nobody_says_nothing_about_it():
    ...
    from comfyui.h3ir_client import plan_assets
...
def test_the_option_lists_the_pack_draws_are_the_ones_the_contract_publishes():
    ...
    from comfyui import h3ir_client as CL
...
def test_the_refusals_the_client_groups_are_all_refusals_the_compiler_still_makes():
    ...
    """
    from comfyui.h3ir_client import REFUSED_AS_ASKED
...
def test_a_service_that_answers_with_something_else_is_treated_as_publishing_nothing():
    ...
    import comfyui.h3ir_client as CL
