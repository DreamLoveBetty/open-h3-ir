...
one of those is settled in Python -- `comfyui/web_api.py` for the extensions and
`comfyui/tray.py` for the capacities -- so the panel now restates three tables it must not be the
...
from comfyui import web_api as W

TRAY_JS = (pathlib.Path(__file__).resolve().parents[1]
           / "comfyui" / "web" / "tray.js").read_text(encoding="utf-8")
...
def test_the_panel_sorts_files_by_the_table_the_route_refuses_by():
    ...
    for kind, allowed in W.EXTENSIONS.items():
        assert declared[kind] == list(allowed), (
            ...
            "comfyui/web_api.py is the authority; tray.js restates it.")
...
def test_the_panel_counts_the_same_room_the_tray_enforces():
    """`CAPACITY` and `MAX_FILES` in tray.js are the ones comfyui/tray.py refuses by.
    ...
    assert declared == T.CAPACITY, (
        f"comfyui/tray.py holds {T.CAPACITY} and the panel counts {declared}")
    ...
    assert int(m.group(1)) == T.MAX_FILES, (
        f"comfyui/tray.py sends at most {T.MAX_FILES} files and the panel counts to {m.group(1)}")
...
def test_the_panel_names_every_kind_the_tray_has_a_slot_for(kind):
    ...
    assert set(words) == set(T.KINDS), (
        f"comfyui/tray.py has slots for {sorted(T.KINDS)} and the panel has words for "
        f"{sorted(words)}")
